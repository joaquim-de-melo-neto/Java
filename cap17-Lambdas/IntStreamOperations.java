...
import java.util.stream.IntStream;ÿ
...
public class IntStreamOperations
{
  public static void main(String[] args)
  {
   ...
   System.out.printf("%nCount: %d%n", ïIntStream.of(values).count());
   System.out.printf("Min: %d%n",
   ïIntStream.of(values).min().getAsInt());
   System.out.printf("Max: %d%n",
   IntStream.of(values).max().getAsInt());
   System.out.printf("Sum: %d%n", IntStream.of(values).sum());
   System.out.printf("Average: %.2f%n",
   ïIntStream.of(values).average().getAsDouble());
